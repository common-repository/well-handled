...
class htmlawed {
	...
	if($C['cdata'] or $C['comment']){$t = preg_replace_callback('`<!(?:(?:--.*?--)|(?:\[CDATA\[.*?\]\]))>`sm', array(static::class, 'hl_cmtcd'), $t);}
	$t = preg_replace_callback('`&amp;([a-zA-Z][a-zA-Z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`', array(static::class, 'hl_ent'), str_replace('&', '&amp;', $t));
	...
	// main
	$t = preg_replace_callback('`<(?:(?:\s|$)|(?:[^>]*(?:>|$)))|>`m', array(static::class, 'hl_tag'), $t);
	...
	// final $spec
	$s = array();
	if(!function_exists(array(static::class, 'hl_aux1'))){function hl_aux1($m){
	return substr(str_replace(array(";", "|", "~", " ", ",", "/", "(", ")", '`"'), array("\x01", "\x02", "\x03", "\x04", "\x05", "\x06", "\x07", "\x08", '"'), $m[0]), 1, -1);
	}}
	$t = str_replace(array("\t", "\r", "\n", ' '), '', preg_replace_callback('/"(?>(`.|[^"])*)"/sm', array(static::class, 'hl_aux1'), trim($t)));
	...
	if(isset($y[$x]['match']) && !static::hl_regex($y[$x]['match'])){unset($y[$x]['match']);}
	if(isset($y[$x]['nomatch']) && !static::hl_regex($y[$x]['nomatch'])){unset($y[$x]['nomatch']);}
	}
	...
	$v = preg_replace_callback('`(url(?:\()(?: )*(?:\'|"|&(?:quot|apos);)?)(.+?)((?:\'|"|&(?:quot|apos);)?(?: )*(?:\)))`iS', array(static::class, 'hl_prot'), $v);
	...
	$v1 = trim($v1[0]);
	if(isset($v1[0])){$v2 .= static::hl_prot($v1, $k). (empty($k1) ? '' : ' '. $k1). ', ';}
	}
	$v = trim($v2, ', ');
	}
	if($k == 'itemtype'){
	...
	if(isset($v1[0])){$v2 .= static::hl_prot($v1, $k). ' ';}
	}
	$v = trim($v2, ' ');
	}
	else{$v = static::hl_prot($v, $k);}
	if($k == 'href'){ // X-spam
	...
	if(isset($rl[$k]) && is_array($rl[$k]) && ($v = static::hl_attrval($k, $v, $rl[$k])) === 0){continue;}
	$a[$k] = str_replace('"', '&quot;', $v);
	}
	}
	...
	if(!function_exists(array(static::class, 'hl_aux2'))){function hl_aux2($m){
	return $m[1]. str_replace(array("<", ">", "\n", "\r", "\t", ' '), array("\x01", "\x02", "\x03", "\x04", "\x05", "\x07"), $m[3]). $m[4];
	}}
	$t = preg_replace(array('`(<\w[^>]*(?<!/)>)\s+`', '`\s+`', '`(<\w[^>]*(?<!/)>) `'), array(' $1', ' ', '$1'), preg_replace_callback(array('`(<(!\[CDATA\[))(.+?)(\]\]>)`sm', '`(<(!--))(.+?)(-->)`sm', '`(<(pre|script|textarea)[^>]*?>)(.+?)(</\2>)`sm'), array(static::class, 'hl_aux2'), $t));
	if(($w = strtolower($w)) == -1){
